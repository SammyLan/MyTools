...
@set BUILD_OPT=RTLIBCFG=static mode=static ENABLE_WINSSL=no VC=%VS_VERSION%  DEBUG=yes _WIN32_WINNT=0x0501
...
@xcopy /S /Y %COPYFROMDIR%include  %LIB3rd_Inc%  >%LOGS_DIR%\xcopyCURL.log
@xcopy /S /Y %COPYFROMDIR%Lib  %LIB3rd_Lib%  >>%LOGS_DIR%\xcopyCURL.log
